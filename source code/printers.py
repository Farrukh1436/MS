...
class PrintersPage(ctk.CTkFrame):
    def __init__(self, master=None, controller=None):
        ...
        image_files = ["custom1.png", "custom2.png", "masung.png"]
        self.tk_images = []
        printers = ["Custom1", "Custom2", "MASUNG"]
        for ba_id, printer in enumerate(printers):
            label = ctk.CTkLabel(self, text=f"{printer} lar soni", font=("Helvetica", 25, "bold"), text_color="#DC143C", fg_color="#020f12")
            label.place(relx=0.2 + ba_id * 0.3, rely=0.2, relwidth=0.2, relheight=0.2, anchor="center")
        ...
        for idx, tk_image in enumerate(self.tk_images):
            ...
            frame.place(relx=0.1 + idx * 0.3, rely=0.3, relwidth=0.2, relheight=0.3)  # Adjust the frame size and position
    ...
    def create_widgets(self):
        ...
        cursor.execute('SELECT CUSTOM1_n,CUSTOM2_n,MASUNG_n  FROM storage WHERE id = 1')
        result = cursor.fetchone()
        c1_value = result[0] if result else 0
        c2_value = result[1] if result else 0
        masung_value = result[2] if result else 0

        # Create entries for each value
        self.entries = []
        values = [c1_value, c2_value,masung_value]
        labels = ["Custom1", "Custom2", "Masung"]
        
        for idx, (value, label) in enumerate(zip(values, labels)):
            entry = ctk.CTkEntry(self, width=100)
            entry.insert(0, str(value))
            entry.place(relx=0.2 + idx * 0.3, rely=0.25, anchor="center")
    ...
    def update_values(self):
        ...
        try:
            cursor = self.db_connection.cursor()
            cursor.execute('UPDATE storage SET CUSTOM1_n=?, CUSTOM2_n=?, MASUNG_n=? WHERE id = 1', tuple(new_values))
            self.db_connection.commit()
            
            update_label = ctk.CTkLabel(self, text="Updated!", font=("Helvetica", 20, "bold"), text_color="green", fg_color="#020f12")
            update_label.place(relx=0.5, rely=0.75, anchor="center")
